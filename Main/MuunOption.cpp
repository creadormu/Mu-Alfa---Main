...
{
	g_pRenderText2.SetTextColor(0xffffffff);
	...
	{
		g_pRenderText2.SetBgColor(160, 0, 0, 255);
	}
	else if (g_Option.TEXT_COLOR_DARKBLUE == TextListColor)
	{
		g_pRenderText2.SetBgColor(0, 0, 160, 255);
	}
	else if (g_Option.TEXT_COLOR_DARKYELLOW == TextListColor)
	{
		g_pRenderText2.SetBgColor(160, 102, 0, 255);
	}
	//else if ( g_Option.TEXT_COLOR_GREEN_BLUE==TextListColor)
	//{
	//pRender_SetBgColor(g_pRenderText(),60, 60, 200, 255);
	//pRender_SetBgColor(g_pRenderText(),0, 255, 0, 255);
	//}
	else
	{
...
void MuunOption::CRenderItem3D(float x, float y, float Width, float Height, int ItemID, int Level, int Excl, int Anc, bool OnMouse) // code hien thi item
{
	glMatrixMode(0x1701);
	glPushMatrix();
	glLoadIdentity();
...
void MuunOption::RenderTipTextList(int sx, int sy, int TextNum, int Tab, int iSort, int iRenderPoint, BOOL bUseBG)
{
	...
	for (int i = 0; i < TextNum; ++i)
	{
		...
		if (TextBold[i])
			g_pRenderText2.SetFont(g_hFontBold);
		else
			g_pRenderText2.SetFont(g_hFont);

		g_TextExtentPoint32W(GetPoint32(), g_pRenderText2.GetFontHDC(), TextList[i], lstrlen(TextList[i]), &TextSize);
	...
	if (g_Option.ITEM_SELECT != NULL)
	{
		...
		float cy = fsy + 5;
		g_Option.CRenderItem3D(cx, cy, 80, 80, g_Option.ITEM_SELECT, 0, 0, 0, true);
		fsy += 90;
	}
	...
	for (int i = 0; i < TextNum; i++)
	{
		...
		else
		{
			...
			g_pRenderText2.RenderText(fsx, fsy, TextList[i], (fWidth - 2), 0, iSort, &TextSize);
			fHeight = TextSize.cy;
		}
		fsy += fHeight * 1.1f;
	}
...
void __thiscall MuunOption::itemtooltiptextScriptLoader(BYTE * thisa, int pchFileName)
{
	char chFileName[200];
	tooltitextScriptLoader(thisa, pchFileName);

	sprintf_s(chFileName, "Data\\Local\\%s\\MuunTooltipText_%s.bmd", (char*)0xE8C5C4, (char*)0xE8C5C4);
...
int __thiscall MuunOption::AttachToolTipForSocketItem(LPVOID thisa, ITEM* Temp, int TextNum)
{
	//Doan nay la danh sach cac pet duoc dua vao Muun Inventory || Tu 6856 => 7127
	//	if (Temp->Type >= 6856 && Temp->Type < 7168)
	if (Temp->Type >= 6856 && Temp->Type < 7172)
	{
		g_Option.ITEM_SELECT = Temp->Type;
...
String MuunOption::GetGlobalMuunText(int Line)
{
...
int MuunOption::AttachToolTipForMuun(ITEM* Temp, int TextNum)
{
	...
	if (Muun != NULL)
	{
		...
		ITEM_ATTRIBUTE Attribute = *(ITEM_ATTRIBUTE*)(pItem_Attribute + 84 * Temp->Type);

		TextNum = AddOptionLine(g_Option.TEXT_COLOR_WHITE, g_Option.Font_Normal, TextNum, 0, "\n");
		TextNum = AddOptionLine(g_Option.TEXT_COLOR_YELLOW, g_Option.Font_Bold, TextNum, 1, Attribute.Name);
		...
		if (Level >= (Muun->Rank + 1))
			...
			TextNum = AddOptionLine(g_Option.TEXT_COLOR_GREEN, g_Option.Font_Normal, TextNum, 0, g_Option.GetGlobalMuunText(30), Level, Muun->Rank + 1);
...
void MuunOption::Init()
{
	...
	SetCompleteHook(0xE8, 0x007E348F, &MuunOption::itemtooltiptextScriptLoader);
}
